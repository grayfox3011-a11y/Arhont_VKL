...
async function createTables(db: any) {
  ...
  db.run(sql`CREATE TABLE IF NOT EXISTS heroes (
  ...
  db.run(sql`CREATE TABLE IF NOT EXISTS battles (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    name_be TEXT,
    name_en TEXT,
    ...
    result_en TEXT,
    belligerents TEXT,
    belligerents_be TEXT,
    belligerents_en TEXT,
    image_url TEXT,
    order_idx INTEGER,
    created_at INTEGER DEFAULT (unixepoch()),
    updated_at INTEGER DEFAULT (unixepoch())
  ...
  db.run(sql`CREATE TABLE IF NOT EXISTS manuscripts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    title TEXT NOT NULL,
    title_be TEXT,
    title_en TEXT,
  ...
  db.run(sql`CREATE TABLE IF NOT EXISTS castles (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    name_be TEXT,
    name_en TEXT,
    ...
    owners_en TEXT,
    defense_system TEXT,
    defense_system_be TEXT,
    defense_system_en TEXT,
    image_url TEXT,
    gallery TEXT,
    order_idx INTEGER,
    created_at INTEGER DEFAULT (unixepoch()),
...
async function seed() {
  ...
  try {
    ...
    console.error("[Manuscripts] ERROR:", e);
  }
  ...
  // ─── Final verification ─────────────────────────────────────────
  console.log("\n=== FINAL VERIFICATION ===");
  const finalHeroes = await db.select({ count: sql<number>`count(*)` }).from(heroes);
  const finalBattles = await db.select({ count: sql<number>`count(*)` }).from(battles);
  const finalEvents = await db.select({ count: sql<number>`count(*)` }).from(timelineEvents);
  const finalManuscripts = await db.select({ count: sql<number>`count(*)` }).from(manuscripts);
